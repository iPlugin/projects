...
bool send_packet(int socketFD){
    ...
    packet.iph.daddr = inet_addr("127.0.0.1");                         // |
                                                                       // |
    packet.tcph.th_sport = htons(2002);                                // |
...
bool recv_packet(int socketFD){
    while (true) {
        ...
        if (packet.iph.daddr == inet_addr("127.0.0.1") && packet.tcph.th_dport == htons(2002) && packet.data != "") {
